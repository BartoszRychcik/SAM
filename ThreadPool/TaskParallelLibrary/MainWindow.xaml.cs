            ...
            while (true)
            {
                ...
                while (_view.FileList.Count > 0)
                {
                    ...
                    try
                    {
                        if (filters[0])
                        {
                            Task t = Task.Factory.StartNew(Edges_Filter_Process);
                            t.Wait();
                        }
                        if (filters[1])
                        {
                            Task t = Task.Factory.StartNew(Erosion_Filter_Process);
                            t.Wait();
                        }
                        if (filters[2])
                        {
                            Task t = Task.Factory.StartNew(Adaptive_Filter_Process);
                            t.Wait();
                        }
                        if (filters[3])
                        {
                            Task t = Task.Factory.StartNew(Oil_Filter_Process);
                            t.Wait();
                        }
                        if (filters[4])
                        {
                            Task t = Task.Factory.StartNew(Jitter_Filter_Process);
                            t.Wait();
                        }
